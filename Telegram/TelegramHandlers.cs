...
namespace MessengerBotObserver.Telegram
{
    public  class TelegramHandlers:ILogging
    {
        ...
        private async Task BotOnMessageReceived(ITelegramBotClient botClient, Message message)
        {
            ...
            LogMessageReceived?.Invoke(nameof(TelegramHandlers), new LogMessage(LogMessageType.Information, message.Text));
            
            await botClient.SendTextMessageAsync(message.Chat.Id, $"**{message.Text}**");

            #region остальной функционал бота пока закомментировал
            /*
